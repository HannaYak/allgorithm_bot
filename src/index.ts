...
bot.action('game_dating', (ctx) => {
  const text = `💘 <b>Быстрые свидания</b>\n\n` +
    `<b>Что это?</b>\n14 человек (7Ж + 7М), 7 столиков, 10-минутные раунды. Бот выдаёт номера, запускает таймер, тусует пары. Отмечай симпатии в карточке — если мэтч, бот пришлёт контакты.\n\n` +
    ...
    `<b>Как это работает?</b>\nИспытайте искру с первого взгляда! Мужчины и женщины садятся по двое за столики, а бот присваивает каждому игроку уникальный номер — и начинается тайминг: каждые 10 минут пары меняются, чтобы вы могли познакомиться с новыми интересными людьми. За время общения вы отмечаете понравившихся собеседников в карточке симпатий. Если чувства взаимны, бот соединит вас — и вы получите контакты друг друга. Готовьтесь к новым встречам и неожиданным открытиям!\n\n` +
    `⏳ <b>Длительность:</b> 1 час 15 минут\n👥 <b>Участников:</b> 14`;
  return ctx.editMessageText(text, { parse_mode: 'HTML', ...Markup.inlineKeyboard([[Markup.button.callback('📅 Записаться', 'book_dating')], [Markup.button.callback('🔙 Назад', 'back_to_games')]]) });
});
