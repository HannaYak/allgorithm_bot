...
bot.action('game_talk', (ctx) => {
  ...
  return ctx.editMessageText(text, { parse_mode: 'HTML', ...Markup.inlineKeyboard([[Markup.button.callback('📅 Записаться', 'book_talk')], [Markup.button.callback('🎲 Темы', 'get_random_topic')], [Markup.button.callback('🔙 Назад', 'back_to_games')]]) });
});

bot.action('game_stock', (ctx) => {
  const text = `🧠 <b>Stock & Know</b>\n\n` +
    ...
    if (STOCK_STATE.currentQuestionIndex !== -1 && !isNaN(parseInt(text)) && !text.startsWith('/')) {
        ...
        if (player) {
            if (!STOCK_STATE.playerAnswers.has(userId)) {
                ...
                // Уведомляем тебя (админа), чтобы ты видела прогресс
            ...
            // Если человек пишет цифры, но не в списке игроков
            return ctx.reply(`❌ Ты не зарегистрирован как участник текущей игры Stock & Know.`);
        }
    }
