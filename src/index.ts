...
bot.hears('🆘 Помощь', (ctx) => {
  (ctx.session as any).waitingForSupport = true;
  ctx.reply('Напиши свой вопрос или описание проблемы прямо сюда, и я передам его администратору! 👇');
});

// Обработчик кнопки "📜 Правила"
bot.hears('📜 Правила', (ctx) => {
  ctx.replyWithHTML(`📜 <b>Правила клуба Allgorithm:</b>\n\n1. Оплата производится заранее через бота.\n2. Отмена записи с возвратом средств/ваучера возможна не позднее чем за 36 часов до игры.\n3. Будьте вежливы и уважайте других участников.`);
});
...
bot.on('message', async (ctx, next) => {
  ...
  if (sess?.waitingForSupport) {
    await ctx.telegram.sendMessage(ADMIN_ID, `🆘 <b>ПОДДЕРЖКА</b>\nОт: ${ctx.from.first_name}\nID: ${ctx.from.id}\nТекст: ${text}\n/reply ${ctx.from.id} `);
    ctx.reply('✅ Отправлено!'); sess.waitingForSupport = false; return;
  }
  return next();
});
