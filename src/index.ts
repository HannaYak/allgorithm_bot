...
bot.hears('🎲 Новая тема', async (ctx) => {
  ...
  // --- 1. ЛОГИКА ДЛЯ СВИДАНИЙ (Оставляем как была) ---
  ...
  // --- 2. ЛОГИКА ДЛЯ ОБЫЧНЫХ И ТЕМАТИЧЕСКИХ ИГР ---
  else {
    // Выбираем пул вопросов
    const pool = (currentEvent.type === 'talk_thematic') ? THEMATIC_QUESTIONS_POOL : CONVERSATION_TOPICS;
    const randomTopic = pool[Math.floor(Math.random() * pool.length)];

    // Получаем всех участников для выбора спикера
    ...
    SPEAKERS_HISTORY[currentEvent.id] = winner.id; // Запоминаем
