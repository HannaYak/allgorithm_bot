...
if (minutesSinceStart >= -210 && minutesSinceStart <= 0 && !(await isProcessed(`reveal_${event.id}`))) {
      ...
      if (event.type === 'tiffany') {
        for (let i = 0; i < TIFFANY_LEVELS.length; i++) {
          const level = TIFFANY_LEVELS[i];
          
          // Если подошло время уровня и мы его еще не запускали
          if (minutesSinceStart >= level.timeTrigger && minutesSinceStart <= level.timeTrigger + 5 && !(await isProcessed(`tiffany_l${i}_${event.id}`))) {
              await markAsProcessed(`tiffany_l${i}_${event.id}`, 20);
              
              const randomQ = level.questions[Math.floor(Math.random() * level.questions.length)];
              await addUsedTopic(event.id, randomQ); // Запоминаем, чтобы не повторялось
...
let usedSet = await getUsedTopics(currentEvent.id);
  ...
  // --- ЛОГИКА ДЛЯ ТИФФАНИ, ОБЫЧНЫХ И ТЕМАТИЧЕСКИХ ---
    ...
    if (currentEvent.type === 'tiffany') {
        // Определяем текущий уровень "Айсберга" по прошедшему времени
        ...
        for (let i = TIFFANY_LEVELS.length - 1; i >= 0; i--) {
            if (minsPassed >= TIFFANY_LEVELS[i].timeTrigger) {
                activeLevelIndex = i;
                break;
            }
        }
        poolRaw = TIFFANY_LEVELS[activeLevelIndex].questions;
        
    } else if (currentEvent.type === 'lockload') {
        poolRaw = THEMATIC_QUESTIONS_POOL; 
    } else {
