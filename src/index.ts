...
async function autoCloseEvent(eventId: number) {
  ...
  for (const b of bookings) {
    ...
    if (u) {
      ...
      bot.telegram.sendMessage(u.telegramId, '🎁 <b>Игра завершена!</b>\n\nМы добавили вам +1 балл лояльности. Каждая 5-я игра в нашем боте — бесплатно! До новых встреч! ✨', { parse_mode: 'HTML' }).catch(() => {});
    }
  }
}
...
const registerScene = new Scenes.WizardScene('REGISTER_SCENE',
  async (ctx) => { 
      await ctx.replyWithHTML(
        `👋 <b>Почти готово!</b>\n\n` +
        `Перед тем как ты забронируешь место, нам нужно внести тебя в базу клуба. Это поможет нам подготовить именные карточки и настроить под тебя игровую механику (особенно для <b>Talk & Toast</b>).\n\n` +
        `⏱ Это займет около минуты и делается всего один раз.\n\n` +
        `<b>Готов(а)? Поехали! 🚀</b>\n\n` +
        `1. Как тебя зовут?`
      );
...
bot.start(async (ctx) => {
  ...
  if (!user) {
      ...
      // 1. Создаем пользователя и СРАЗУ получаем его данные из базы
      ...
      }).returning(); // Это важно, чтобы получить newUser.id

      // 2. Если человек пришел по ссылке — выдаем ему ваучер на -10 PLN
      if (referrerId) {
      ...
      try {
          ...
          // Если видео всё еще выдает ошибку 400, бот просто пойдет дальше и не упадет
      }

      // 4. Формируем текст приветствия
      let welcomeText = `👋 <b>Привет! Рады видеть тебя в Allgorithm.</b>\n\n`;
      
      if (referrerId) {
          welcomeText += `🎁 <b>Тебе начислена скидка 10 PLN на первую игру!</b> Она применится автоматически при бронировании билета.\n\n`;
      }

      welcomeText += `Мы верим, что окружение определяет результат, поэтому здесь нет случайных людей.\n\n` +
        `<b>Что тебя ждет:</b>\n` +
        `🥂 <b>Talk & Toast</b> — ужины без «small talk».\n` +
        `🧠 <b>Stock & Know</b> — интеллектуальный азарт.\n` +
        `💘 <b>Fast Dates</b> — 7 знакомств за час.\n\n` +
...
bot.hears('👤 Личный кабинет', async (ctx) => {
    ...
    // 1. Проверяем, заполнена ли анкета
    const isProfileComplete = !!user.name;

    // 2. Считаем доступные ваучеры
    ...
    message += `🎫 <b>Доступные скидки:</b> ${vouchers.length} шт. (по 10 PLN)\n`;
    message += `👥 <b>Приглашено друзей:</b> ${user.invitedCount || 0}\n\n`;

    if (!isProfileComplete) {
        message += `⚠️ <b>Твой профиль не заполнен!</b>\nБез анкеты Алгоритм не сможет подобрать тебе идеальную компанию для игр. Заполни её прямо сейчас. 👇`;
    } else {
    ...
    // Кнопка анкеты (Регистрация или Изменение)
    if (!isProfileComplete) {
        buttons.push([Markup.button.callback('📝 Заполни анкету (3 мин)', 'start_registration')]);
    } else {
        buttons.push([Markup.button.callback('✏️ Изменить анкету', 'start_registration')]);
    }

    // --- ВОТ ЭТА КНОПКА БЫЛА ПРОПУЩЕНА ---
    buttons.push([Markup.button.callback('📸 У меня есть ваучер', 'upload_voucher')]);

    // Кнопка «Мои записи» (Исправлено: callback 'my_games', чтобы совпадал с кодом ниже)
    buttons.push([Markup.button.callback('🎮 Мои записи на игры', 'my_games')]);
...
// Действие для реферальной ссылки
bot.action('invite_friend', async (ctx) => {
    ...
    await ctx.reply(
        `📢 <b>Твоя реферальная ссылка:</b>\n\n<code>${refLink}</code>\n\nОтправь эту ссылку другу. Если он зарегистрируется и купит билет, тебе придет бонус <b>-10 PLN</b>!`,
        { parse_mode: 'HTML' }
    );
...
bot.action('referral_info', async (ctx) => {
  ...
  const message = `<b>🤝 Приглашай друзей — играйте со скидкой!</b>\n\n` +
    `Твоя персональная ссылка:\n<code>${refLink}</code>\n\n` +
    `🎁 <b>Как это работает:</b>\n` +
    `1. Твой друг переходит по ссылке и <b>сразу получает -10 PLN</b> на первую игру.\n` +
    `2. Как только друг оплатит свой первый билет, <b>ты тоже получишь ваучер на -10 PLN</b> на свою следующую игру!`;
...
bot.action(/exec_canc_(\d+)/, async (ctx) => {
    ...
    try {
        ...
        if (start.diff(now, 'hours').hours < 36) {
            return ctx.editMessageText('⚠️ Слишком поздно. По правилам Алгоритма отмена через бота возможна только за 36 часов.');
        }
...
// ЧАСТЬ 1: СОЗДАНИЕ СЧЕТА (pay_event)
bot.action(/pay_event_(\d+)/, async (ctx) => {
    ...
    try {
        ...
        // 2. Логика каждой 5-й бесплатной игры
        ...
        // 3. Проверка активных ваучеров (approved_10 или approved_free)
        ...
        // Применяем скидку -10 PLN, если есть ваучер
        ...
        // 5. Создание сессии оплаты (ФОРСИРУЕМ BLIK)
        const stripeSession = await stripe.checkout.sessions.create({
            // Явно перечисляем методы, чтобы BLIK не "прятался"
            payment_method_types: ['card', 'blik', 'revolut_pay'],
            line_items: [{
                price: priceId, // Обязательно в PLN в Stripe!
                quantity: 1,
            }],
            metadata: sessionMetadata,
            discounts: discounts,
            mode: 'payment',
            locale: 'ru', // Чтобы интерфейс оплаты был на русском
        ...
        await ctx.reply(msg, { 
            parse_mode: 'HTML', 
            ...Markup.inlineKeyboard([
                [Markup.button.url('💸 Оплатить (Карта, BLIK, Revolut)', stripeSession.url)], 
        ...
        ctx.reply(`Ошибка платежной системы. Убедитесь, что цена в Stripe создана в PLN.`); 
    }
});

// ЧАСТЬ 2: ПОДТВЕРЖДЕНИЕ ОПЛАТЫ (confirm_pay)
bot.action(/confirm_pay_(\d+)/, async (ctx) => {
    ...
    try {
        // Проверяем статус в Stripe (лимит 15 сессий для надежности)
        ...
        if (!paidSession) return ctx.reply('🔍 Оплата не найдена. Подождите 10 секунд и попробуйте снова.');
        ...
        // Если была скидка по ваучеру — помечаем его использованным
        ...
        // Фиксируем бронь в базе
        await db.insert(schema.bookings).values({ userId: user.id, eventId: eventId, paid: true });

        // РЕФЕРАЛЬНАЯ ЛОГИКА: Начисляем бонус пригласившему (Друг 1)
        if (user.invitedBy) {
            ...
            if (inviter) {
                ...
                bot.telegram.sendMessage(inviter.telegramId, 
                    `🎉 <b>Твой бонус за друга!</b>\n\nТвой друг ${user.name} оплатил первую игру. Тебе начислен ваучер на <b>-10 PLN</b> на следующую встречу!`,
                    { parse_mode: 'HTML' }
                ).catch(()=>{});

                // Очищаем invitedBy, чтобы бонус был только за одну (первую) игру
        ...
        ctx.editMessageText('🎉 Оплата подтверждена!\nВы в игре! 😎\n\nМесто встречи откроется за 3 часа. Я напомню Вам заранее об участии.', { parse_mode: 'HTML' });

    } catch (e) { 
        console.error('Confirm Pay Error:', e);
        ctx.reply('Ошибка при проверке оплаты. Попробуйте позже или напишите админу.'); 
    }
});
// --- 9. ВАУЧЕРЫ ---
...
bot.on('message', async (ctx, next) => {
    ...
    // Базовая проверка: если нет ID пользователя, ничего не делаем
    if (!userId) return next();

    // -------------------------------------------------------------------------
    // ФУНКЦИЯ 1: ЛОГИКА РАССЫЛКИ (Только для Админа)
    // -------------------------------------------------------------------------
    // Мы ставим её ПЕРВОЙ, чтобы бот мог скопировать кружочек или фото до того,
    // как он начнет искать в сообщении текст.
    if (sess?.waitingForBroadcast && userId === ADMIN_ID) {
        ...
        for (const u of users) {
            try { 
                // Используем copyMessage — это самый мощный метод, он копирует 
                // кружочки, видео, фото и текст один в один.
                await ctx.telegram.copyMessage(u.telegramId, ctx.chat!.id, msg.message_id);
                ok++; 
            } catch(e) {
                // Если кто-то заблокировал бота, просто пропускаем его
            }
            // Задержка 50мс между сообщениями, чтобы Telegram не забанил бота
    ...
    // -------------------------------------------------------------------------
    // ПОДГОТОВКА ТЕКСТА ДЛЯ ОСТАЛЬНЫХ ФУНКЦИЙ
    // -------------------------------------------------------------------------
    const text = msg.text;

    // Если в сообщении нет текста (и это НЕ рассылка) — выходим.
    // Это защищает бота от ошибок, если ему пришлют стикер просто так.
    if (!text) return next();

    // Если это команда (начинается с /), пропускаем её к специальным обработчикам
    if (text.startsWith('/')) return next();


    // -------------------------------------------------------------------------
    // ФУНКЦИЯ 2: ЛОГИКА ПРИЕМА ОТВЕТОВ В ИГРЕ STOCK & KNOW
    // -------------------------------------------------------------------------
    ...
    // -------------------------------------------------------------------------
    // ФУНКЦИЯ 3: ЛОГИКА ТЕХПОДДЕРЖКИ (Для пользователей)
    // -------------------------------------------------------------------------
